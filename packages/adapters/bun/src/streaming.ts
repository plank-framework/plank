...
function createBackpressureStream(stream: ReadableStream<Uint8Array>): ReadableStream<Uint8Array> {
  return new ReadableStream({
    start(controller) {
      ...
      const processStream = async (reader: ReadableStreamDefaultReader<Uint8Array>, controller: ReadableStreamDefaultController<Uint8Array>): Promise<void> => {
        while (true) {
 ...
 */
export function stringToStreamResponse(
  content: string,
  options: StreamingOptions = {}
): Response {
